...
export const isValidEmailOrUsername = text => {
  const re =
    /^(?!^[@\\.\\_\\-\\d])(?!.[@\.\_\-]$)(?!.[@\.\_\-]{ 2,})(?!.@+(?![a-zA-Z0-9-]\.+[a-zA-Z0-9]{2,4}))^([a-zA-Z0-9\-\_@.]{4,64})+$/;
  return re.test(text);
};
...
export const isNationalCode = (
  text,
  {typing = false, foreign = false} = {},
) => {
  if (!text.length) {
    return false;
  }
  ...
  let foreignCitizensCodeRegEx =
    /^[0-9۰-۹آابپتثجچحخدذرزژسشصضطظعغفڤقکگلمنوهیءكةئأإؤي]{10,15}$/;
  ...
  if (typing === true) {
    regex =
      foreign === false
        ? /^[0-9۰-۹]{1,10}$/
        : /^[0-9۰-۹آابپتثجچحخدذرزژسشصضطظعغفڤقکگلمنوهیءكةئأإؤي]{1,15}$/;
  } else {
...
export const isPersianName = (
  text,
  {typing = false, maxLength = MAX_NAME_LENGTH} = {},
) => {
  if (!text) {
    return false;
  }
...
export const isPersianNameWithDigits = (
  text,
  {typing = false, maxLength = MAX_NAME_LENGTH} = {},
) => {
  if (!text) {
