...
export default function Expense() {
  ...
  const [expenseSourceModalVisibility, setExpenseSourceModalVisibility] =
    useState(false);
  ...
  const [expenseTypeDropDownVisibility, setExpenseTypeDropDownVisibility] =
    useState(false);
    ...
    database
      ...
      .then(res => {
        let tempExpense = [],
          tempRecurring = [];
        for (let ele of res) {
          if (ele.type == 0) tempExpense.push(ele);
          else tempRecurring.push(ele);
        }
        setExpensesList(tempExpense);
        setRecurringExpensesList(tempRecurring);
      });
  }, [expensesList.length, recurringExpensesList.length]);
    ...
    return (
      <View
        ...
        <Text
          ...
          No Source of Expense Found
        </Text>
        <Pressable
          android_ripple={{
            color: '#ededed',
          }}
          onPress={onNewExpenseSourceButtonPressed}
          style={{
  ...
  const onNewExpenseSourceButtonPressed = _ => {
    setExpenseSourceModalVisibility(true);
  };
  ...
  const onNewExpenseSubmitted = _ => {
    ...
    database.write(async () => {
      const newExpense = await database.collections
        .get('expenses')
        .create(res => {
          res.source = expenseSource;
          if (expenseTypeId == 1) res.frequency = frequency;
          res.amount = parseFloat(amount);
          res._raw.date = newDate.unix();
          res.type = parseFloat(expenseTypeId);
        });
      database
        .get('expenses')
        .query()
      ...
      setExpenseTypeId(0);
      setDate(moment().unix());
      setExpenseSourceModalVisibility(false);
    });
  };
  ...
  return (
    <View
      ...
      <Modal
        animationType="fade"
        visible={expenseSourceModalVisibility}
        onRequestClose={_ => setExpenseSourceModalVisibility(false)}>
        <View
          style={{
            backgroundColor: 'white',
            height: deviceHeight,
                ...
                <Pressable
                  ...
                  <View
                    style={{
                      ...
                      height: 10,
                      borderRadius: 60,
                      backgroundColor: expenseTypeId == 0 ? '#5CD598' : 'white',
                    }}></View>
                </Pressable>
              </Pressable>

              <Pressable
                onPress={_ => setExpenseTypeId(1)}
                style={{
                  justifyContent: 'space-between',
                  alignItems: 'center',
                  flexDirection: 'row',
                ...
                <Pressable
                  ...
                  <View
                    style={{
                      ...
                      height: 10,
                      borderRadius: 60,
                      backgroundColor: expenseTypeId == 1 ? '#5CD598' : 'white',
                    }}></View>
                </Pressable>
              </Pressable>
            </View>
          ...
          <FlotableInput
            ...
            value={expenseSource}
            setValue={setExpenseSource}
            inputPlaceholder="Source of Expense"
          />
          <FlotableInput
            inputStyle={{width: '90%', alignSelf: 'center'}}
      ...
      <FlatList
        ...
        data={activeTab == 0 ? expensesList : recurringExpensesList}
        renderItem={renderItem}
        ListEmptyComponent={renderListEmptyComponent}
      />

      {(expensesList.length && activeTab == 0) ||
      (recurringExpensesList.length && activeTab == 1) ? (
        <Pressable
          android_ripple={{
            color: '#ededed',
          }}
